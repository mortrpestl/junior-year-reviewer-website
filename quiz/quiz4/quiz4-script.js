...
function submittedCheckerFunction(){
    ...
    if(t4Submitted == "true"){
        document.getElementById("submittedChecker").innerHTML = 'STATUS: Part 4 Submitted -- you can still change your answers by changing them and resubmitting';
        document.getElementById("submittedChecker").style.color = 'green';
    }
    else{
        document.getElementById("submittedChecker").innerHTML = 'STATUS: Part 4 NOT SUBMITTED -- please submit with the "Submit" button below';
...
function progressSaverOnunload() {
    localStorage.setItem('t4q1Value', document.getElementById("t4q1_answer").value);
    localStorage.setItem('t4q2Value', document.querySelector('input[name="t4q2_answer"]:checked').value);
    localStorage.setItem('t4q3Value', document.getElementById("t4q3_answer").value);
    localStorage.setItem('t4q4Value', document.querySelector('input[name="t4q4_answer"]:checked').value);
    localStorage.setItem('t4q5Value', document.getElementById("t4q5_answer").value);
}

// this function loads your progress upon connection
function progressSaverOnload() {
    var LSq1Value = localStorage.getItem('t4q1Value');
    var LSq2Value = localStorage.getItem('t4q2Value');
    var LSq3Value = localStorage.getItem('t4q3Value');
    var LSq4Value = localStorage.getItem('t4q4Value');
    var LSq5Value = localStorage.getItem('t4q5Value');

    var q2List = document.querySelectorAll('input[name="t4q2_answer"]');
    var q4List = document.querySelectorAll('input[name="t4q4_answer"]');
    
    // setting the defaults for the user using his past data
    document.getElementById("t4q1_answer").value = LSq1Value;
    for (i = 0; i < q2List.length; i++) {
        if (q2List[i].value == LSq2Value) {
            q2List[i].checked = true;
        }
    }
    document.getElementById("t4q3_answer").value = LSq3Value;
    for (i = 0; i < q4List.length; i++) {
        if (q4List[i].value == LSq4Value) {
            q4List[i].checked = true;
        }
    }
    document.getElementById("t4q5_answer").value = LSq5Value;
}
...
function submitFunction() {
    ...
    if (dataValidated) {
        ...
        var q1Value = document.getElementById("t4q1_answer").value;
        var q2Value = document.querySelector('input[name="t4q2_answer"]:checked').value;
        var q3Value = document.getElementById("t4q3_answer").value;
        var q4Value = document.querySelector('input[name="t4q4_answer"]:checked').value;
        var q5Value = document.getElementById("t4q5_answer").value;
        q1Value = q1Value.trim();
        q3Value = q3Value.trim();
        ...
        if (q1Value.indexOf("(-x)") != "-1" &&
            q1Value.indexOf("(-x+3)") != "-1" &&
            q1Value.indexOf("(-x-1)") != "-1" &&
            q1Value.indexOf("(-x+2)^2") != "-1") 
            {
            console.log("a");
            t4Total += 1;
        };

        if (q2Value == "cube root") {
            t4Total += 1;
        }
        ...
        if (confirmVariable) {
            localStorage.setItem('t4Total', t4Total);
            localStorage.setItem('t4Submitted', "true");
            return true;
        }
        else {
            return false;
...
function resetFunction() {
    ...
    if (confirmVariable) {
        ...
        localStorage.setItem('t4Total', "");
        localStorage.setItem('t4Submitted', "");
